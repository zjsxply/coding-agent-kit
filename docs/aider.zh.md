...
| `AIDER_OPENAI_BASE_URL` | OpenAI 兼容 base URL（回退：`OPENAI_BASE_URL`） | 可选 |
...
- 对未知的 OpenAI 兼容 `kimi-*` 模型，cakit 会写入运行期 `--model-metadata-file`，为该模型补充 `supports_vision: true`，避免 Aider 按纯文本模型处理图片。
