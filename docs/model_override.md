...
For OpenAI-compatible API agents, model resolution is:
- `--model`
- then agent-specific model env key in this table
- then shared `OPENAI_DEFAULT_MODEL`
...
| `opencode` | `CAKIT_OPENCODE_MODEL` | `--model <provider/model>`; when `CAKIT_OPENCODE_OPENAI_BASE_URL` is set, cakit also injects provider `baseURL` via `OPENCODE_CONFIG_CONTENT` |
