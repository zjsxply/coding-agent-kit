...
- `0`：工具安装流程以 best-effort 模式完成，且至少有一个目标工具可用；被跳过/失败的组件会在最终 JSON 中报告。
- `1`：平台/前置条件阻止流程继续，或没有任何目标工具成功安装。

`cakit tools` 会对成功步骤保持安静；若单个工具安装失败，也会继续尝试剩余工具，并在最终 JSON 输出中返回 `installed` / `skipped` / `failed` 汇总。
