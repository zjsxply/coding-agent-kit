...
部分 agent 现在会先尝试官方 shell 安装器，只有该路径失败时才回退到 npm；对这些 agent 来说，`--scope` 只有在触发 npm 回退时才会生效。
...
- `codex` / `codebuddy` / `gemini` / `qwen` / `qoder` / `continue` / `crush` / `auggie` / `kilocode` / `kimi`：npm 包版本号或 tag（例如 `0.98.0`、`2026.2.15`、`1.9.0`）。
- `claude`：Claude Code 安装脚本的版本选择器（例如 `stable`，或 Anthropic 安装脚本支持的精确 Claude Code 版本）。cakit 会先尝试官方脚本安装；如果脚本路径失败，再回退到已 deprecated 的 npm 包 `@anthropic-ai/claude-code`。
- `copilot`：Copilot 安装脚本使用的 `VERSION` 值（例如 `1.0.3`）。cakit 会先尝试官方安装器；如果脚本路径失败，再回退到 npm 包 `@github/copilot`。
- `openclaw`：OpenClaw 安装脚本的 `--version` 取值（例如 `1.0.0`）。cakit 会先尝试官方安装器；如果脚本路径失败，再回退到 npm 包 `openclaw`。
- `opencode`：OpenCode 安装脚本的 `--version` 取值（例如 `0.0.8`）。cakit 会先尝试官方安装器；如果脚本路径失败，再回退到 npm 包 `opencode-ai`。
...
支持的包管理器：`apt-get`、`apk`、`dnf`、`microdnf`、`yum`、`zypper`、`pacman`。
在 apt 系发行版上，cakit 还会尝试安装 Playwright Chromium 的运行依赖和浏览器；在其他 Linux 发行版上，Playwright Chromium 可能会被跳过，但其余工具仍会继续安装。
...
- [ ] `cakit tools` 的 Playwright 支持扩展到其它 Linux 发行版
- [ ] 适配 ARM
